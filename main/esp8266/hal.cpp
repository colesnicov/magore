...
// mux XPD_DCDC to rtc_gpio0
	WRITE_PERI_REG(PAD_XPD_DCDC_CONF, (READ_PERI_REG(PAD_XPD_DCDC_CONF) & (uint32_t)0xffffffbc) | (uint32_t)1L);

// mux out enable
	WRITE_PERI_REG(RTC_GPIO_CONF, (READ_PERI_REG(RTC_GPIO_CONF) & (uint32_t)0xfffffffe) | (uint32_t)0L);

// out enable
	WRITE_PERI_REG(RTC_GPIO_ENABLE, (READ_PERI_REG(RTC_GPIO_ENABLE) & (uint32_t)0xfffffffe) | (uint32_t)out);
}
...
//	GPIO_PIN_HI(pin);
//	GPIO_PIN_MODE(pin);
	ESP_LOGD(__FILE__, "chip init...%d", pin);
	if (pin == 16) {
		GPIO16_PIN_HI();
		GPIO16_PIN_MODE();
	} else {
	...
	ESP_LOGD(__FILE__, "chip init...OK");
}
...
//	ESP_ERROR_CHECK(spiSemaphore == NULL);
	if (spiSemaphore != NULL) {
		xSemaphoreGive(spiSemaphore);
//		ESP_LOGE("HAL", "Semafor vytvoren");

		return true;
	}
	ESP_LOGE("HAL", "NEMUZU VYTVORIT");
	return false;
}
/** 
